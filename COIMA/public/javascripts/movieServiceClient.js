...
window.onload = function() {
    ...
    //     .forEach(div => {
    //         const btn = div.querySelector('#reply')
    //         const currentReplyUser = div.querySelector('.current-ReplyUser')
    //         const currentReplyId = div.querySelector('.current-ReplyId')
    //         btn.addEventListener('click', () => {
    //             if(userComment.value){
    //                 const data =  `username=${currentUser.value}&movieId=${currentMovieId.value}&comment=${userComment.value}&title=${currentTitle.value}&replyUser=${currentReplyUser.value}&replyId=${currentReplyId.value}`
    ...
    document
        ...
        .forEach(div => {
            ...
            btn.addEventListener('click', () => {
                ...
                const data = `listName=${lstName.value}`
                if(lstName.value)
    ...
    function sendListNewName(editRequestButton, div){
        editRequestButton.onclick = function () {
            ...
            httpRequest('POST', '/updateListName', data, (err, body) => {
                if(err) return alert(err)
    ...
    function deleteUserList(deleteButton, div){
        deleteButton.onclick = function () {
            ...
            httpRequest('DELETE', '/deleteList', data, (err, body) => {
                if(err) return alert(err)
                alert('List Deleted!')
                id--
                div.parentNode.removeChild(div)
